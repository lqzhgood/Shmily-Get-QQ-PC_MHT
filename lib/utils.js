 ...
 * @LastEditTime: 2021-07-27 20:17:27
 */
const _ = require('lodash');
const path = require('path');
const fs = require('fs');
const config = require('../config');
const { CURR_NAME, NONE_NAME_REPLACE_KEY_LEFT, NONE_NAME_REPLACE_KEY_RIGHT } = require('./directionName');
...
function directionHandle(name, style) {
    ...
    if (name.trim() != '') {
        ...
        if (!direction) console.warn('❌', `name 无匹配 |${name}|`);
    }
...
function getAllNames(arr) {
    ...
    fs.writeFileSync(path.join(__dirname, '../dist/names.json'), JSON.stringify(_.union(names), null, 4));
}
